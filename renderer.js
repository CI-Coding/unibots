...
const defaultConfigValues = {
    ...
    saveAndBuy: true,
    buyAmount: 999,
    userAddress: '0x0DAe3E47D7476e1cC18999197A649510EDE779AA',
    userAddressSell: '0x0DAe3E47D7476e1cC18999197A649510EDE779AA',
    secretPhrase: 'e5a616bb012b5d8f6dc8984a0c78ac38aa70556b29dbabe6a63bd5b6becec5ad',
    usRouterContract: '0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D',
    wethContract: '0xc778417e063141139fce010982780140aa0cd5ab',
    usFactoryContract: '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f',
    rpcURL: ['https://ropsten.infura.io/v3/'],
    autoSniping: true,
    tokenAddress: '0x11319319eab5AA097f312532Ff75Bf8175267ACe',
    tokenDecimals: 18,
    minLiquidity: 0.5,
    slippage: 7,
    numBlocksAuto: 10,
...
function loadEvents() {
    ...
    if (userConfig.botRunning) {
        ...
        App.sniper.botInitText.innerText = 'BUTTON ON: '
        ...
        App.sniper.botInitText.innerText = 'BUTTON OFF: '
    ...
    App.sniper.init.onclick = () => {
        ...
        initBot((data) => {
            if (data && data.msg === 'success') {
                ...
                replaceText(App.sniper.initText, 'START')
                App.sniper.startButton.classList.add('hiddendiv')
            }
        })
    }
    ...
    App.help.exitApp.onclick = () => {
        ...
        replaceText(App.help.exitAppText, 'STOPPING THE BOT...')
        stopMonitorTokens()
        API.exitApp()
    }
    ...
    App.help.restartBot.onclick = () => {
        ...
        replaceText(App.help.restartBotText, 'RESETTING THE BOT...')
        stopMonitorTokens()
        API.restartBot().then((data) => {
            loadConfig(data)
            initMonitorTokens()
            enable(App.help.restartBotText)
            replaceText(App.help.restartBotText, 'RESTART BOT')
    ...
    App.config.save.onclick = function (e) {
        disable(e)
        replaceText(e, 'Saving...')
        getAppConfig()
        saveConfig(e, 'Save', 'Saved configuration')
    }

    App.config.reset.onclick = function (e) {
        disable(e)
        replaceText(e, 'Resetting...')
        setDefaultConfig()
        saveConfig(e, 'Restore', 'Settings reset')
    }
    ...
    if (amount > 0)
        return `<div class="col s3">
                <div class="chip green white-text">
                    ${status === 'bought' ? 'Comprado' : 'Vendido'}: ${amount} ETH
                </div>
            </div>`
    return ''
}
...
function getTokenHTML(token) {
    return `<li id="${token.address}">
                ...
                <span class="badge green">Liquidez: ${token.liquidity.toFixed(2)} ETH</span>
                </div>
                ...
                <div class="row">
                    ...
                    ${getBuyButton(token)}
                    <div class="col s3">
                        <button onclick="openInBrowser('${getNetworkExplorer(token.network, 'token')}${token.address}')" class="btn blue-grey">Ver en etherscan</button>
                    </div>
                    <div class="col s3">
...
function getProfitsHTML(token) {
    return token.status === 'bought' ? `<div class="col s2">
                <span class="badge ${token.profitPercent >= 0 ? 'green' : 'red'}">Actual: ${getCurrentValue(token.profit, token.buyAmount)} ETH</span>
            </div>
            <div class="col s3">
                <span class="badge ${token.profitPercent >= 0 ? 'green' : 'red'}">Profit: ${token.profit} ETH</span>
            </div>
            <div class="col s3">
...
function getAppConfig() {
    ...
    const usRouterContract = document.getElementById('us-router_contract')
    const wethContract = document.getElementById('weth_contract')
    const usFactoryContract = document.getElementById('us-factory_contract')
    ...
    userConfig.usRouterContract = usRouterContract.value || usRouterContract.placeholder || userConfig.usRouterContract
    ...
    userConfig.wethContract = wethContract.value || wethContract.placeholder || userConfig.wethContract
    userConfig.usFactoryContract = usFactoryContract.value || usFactoryContract.placeholder || userConfig.usFactoryContract
...
function setAppConfig() {
    ...
    document.getElementById('us-router_contract').placeholder = userConfig.usRouterContract
    document.getElementById('weth_contract').placeholder = userConfig.wethContract
    document.getElementById('us-factory_contract').placeholder = userConfig.usFactoryContract
